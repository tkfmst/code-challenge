...
object Solution {
  def romanToInt(s: String): Int = {
    @tailrec
    def f(remaining: String, acc: Int = 0): Int = {
      val r = "^(M|CM|D|CD|C|XC|L|XL|X|IX|V|IV|I)".r
      val m = r.findFirstIn(remaining)
      val i = m match {
        case Some("M")  => 1000
        case Some("CM") => 900
        case Some("D")  => 500
      ...
      if (i == 0) acc
      else f(remaining.drop(m.getOrElse("").length), acc + i)
    }
